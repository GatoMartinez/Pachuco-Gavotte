...
double l( std::vector<double> a, std::vector<double> b, std::vector<double> c, std::vector<double> g, double x_min, double x_max);
double l( std::vector<double> a, std::vector<double> b, std::vector<double> c, std::vector<double> g,double x_min, double x_max){
	int m= a.size();
	...
	if (a.size()!=c.size() or b.size()-a.size()!=1 or b.size()!=g.size()){    //This part of the code is run to make sure that the data have been introduced correcticly
		...
		for (int i = 1 ; i <= m ; i++){
			...
			b1[i]= b[i]-(a[i-1]/b1[i-1])*c[i-1];                  //For loop that redefines the values of b and g after performing the Gauss elimination method
			
			g1[i]= g[i]-(a[i-1]/b1[i-1])*g1[i-1];
